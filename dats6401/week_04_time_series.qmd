...
**Block 1 — Concepts & Theory (~35 min).** What's special about time (9) · trend vs. seasonality; decomposition (12) · visualizing uncertainty (12) · the truncated-axis problem (2).

**Block 2 — Guided Demonstration (~35 min).** Getting dates into pandas: `to_datetime`, DatetimeIndex, time-based vs row-based windows (6) · resample and resolution (7) · ACF, reading the period, comparing rolling windows (8) · decompose; additive vs multiplicative from the residual (8) · uncertainty bands (6).

**☕ Break (~15 min).**

**Block 3 — Hands-On (~50 min).** Parse dates into a DatetimeIndex; two resolutions (12) · ACF for the period, then compare three rolling windows (13) · decomposition and the additive/multiplicative choice (13) · wrap into a Streamlit app with a resolution selector (12).

**Block 4 — Wrap-Up (~15 min).** Share charts; how did resolution change the story? (9) · preview homework (6).

---

### Materials: [Block 2 demo](weekly/week4/week04_block2_demo.ipynb) · [hands-on](weekly/week4/week04_block3_handson.ipynb) · [resolution app starter](weekly/week4/app_resolution_starter.py) · [rolling-window animation](weekly/week4/app_rolling_animation.py)
...
### Animate it in a Streamlit app

A rolling mean is easier to understand when you watch the window move. The full
app is in this week's materials — [`app_rolling_animation.py`](weekly/week4/app_rolling_animation.py):

```bash
streamlit run app_rolling_animation.py
```

It slides the window along the CO₂ series and draws the smoothed value as it
goes, with window length as a slider. Set the window to 3, then 12, then 60, and
watch what each one keeps.

The transferable part is the loop: draw into a placeholder, then overwrite it.
...
Two practical notes. **Redraw cost sets the pace** — each frame re-renders the
figure and sends an image to the browser — so use fewer, larger steps rather than
adding a `sleep`. And put the loop inside an **`@st.fragment`**: otherwise a
button click reruns the whole script, and Streamlit leaves the previous chart on
screen, greyed out, for the entire animation.
